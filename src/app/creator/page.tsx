...
import { CurveType, formatAddress } from '@/config/contracts-v2';
...
export default function CreatorDashboard() {
  ...
  const { tokens: creatorTokens, isLoading: isLoadingTokens, refetch } = useGetCreatorTokens(walletAddress || '');
  ...
  const { createToken, isLoading: isCreating } = useCreateToken();
  const [isProcessing, setIsProcessing] = useState(false);

  const shortAccount = walletAddress ? formatAddress(walletAddress) : '';
  ...
  const allCreatorTokens = creatorTokens || [];
  ...
  const handleCreateToken = async () => {
    ...
    try {
      ...
      const creationFeeAmount = creationFeeValue ? formatEther(creationFeeValue) : '0.01';
      ...
      if (!tokenAddr) {
        throw new Error('Deployed token address missing in event');
      }
    ...
    <div className="page-shell">
      <div className="section-wrap">
        <PageHeader
          ...
          subtitle="Launch and manage your bonding-curve tokens with production-grade controls."
          icon={Rocket}
        />
        ...
        {activeTab === 'create' && (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <Card variant="elevated" padding="lg">
              <h2 className="text-2xl font-bold text-slate-900 mb-6">Create Bonding Curve Token</h2>
              <div className="space-y-4">
                ...
                <p className="text-xs text-neutral-500 -mt-2">
                  Stored as metadata only in this version. Supply is minted dynamically by the bonding curve.
                </p>
                ...
                <div>
                  ...
                  {!tokenImage ? (
                    ...
                    <div className="relative border-2 border-primary-500 rounded-lg p-4">
                      ...
                      <button
                        onClick={removeImage}
                        className="absolute top-2 right-2 p-2 bg-error-500 hover:bg-error-600 text-slate-900 rounded-full transition-colors"
                      >
            ...
            <div className="space-y-6">
              <Card variant="elevated" padding="lg">
                ...
                <div className="space-y-4">
                  ...
                  <div className="text-center">
                    <h4 className="text-2xl font-bold text-slate-900">{tokenName || 'Token Name'}</h4>
                  ...
                  {(website || twitter || telegram) && (
                    <div className="flex gap-2 justify-center">
                      ...
                      {telegram && <span className="text-xs text-neutral-500">Telegram</span>}
                    </div>
                  )}
                </div>
              </Card>
              ...
              <Card variant="elevated" padding="lg">
                <h3 className="text-xl font-bold text-slate-900 mb-4">Curve Info</h3>
                <div className="space-y-3 text-sm">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="text-slate-900">{initialPrice || '0.001'} MATIC</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-neutral-400">Initial Supply</span>
                    <span className="text-slate-900">{Number(initialSupply).toLocaleString('en-US')}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-neutral-400">Creation Fee</span>
                    <span className="text-slate-900">{creationFeeDisplay} MATIC</span>
                  </div>
                </div>
              </Card>
        ...
        {activeTab === 'manage' && (
          <Card variant="elevated" padding="lg">
            <h2 className="text-2xl font-bold text-slate-900 mb-6">Your Tokens</h2>
            {!walletAddress ? (
              <div className="rounded-xl border border-dark-border-primary bg-dark-bg-secondary p-6 text-center space-y-2">
              ...
              <div className="space-y-4">
                {isLoadingTokens && (
                  <div className="rounded-xl border border-dark-border-primary bg-dark-bg-secondary p-6 text-center">
                ...
                {!isLoadingTokens && allCreatorTokens.length > 0 && (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {allCreatorTokens.map((token) => (
                      <div
                        key={token}
                        className="space-y-3 rounded-xl border border-dark-border-primary bg-dark-bg-secondary p-5"
                      >
                        <div className="flex items-center justify-between">
                          <div>
                            <p className="text-sm text-neutral-400">Token Address</p>
                            <p className="text-slate-900 font-mono text-sm">{formatAddress(token)}</p>
                          </div>
                        ...
                        <div className="flex gap-3">
                          ...
                          <a
                            ...
                            target="_blank"
                            rel="noreferrer"
                            className="flex-1 text-center px-3 py-2 rounded-lg bg-dark-bg-hover text-neutral-300 text-sm font-medium hover:text-slate-900 transition"
                          >
                            Polygonscan
                          </a>
                        </div>
        ...
        {activeTab === 'analytics' && (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <Card variant="elevated" padding="lg">
              ...
              <p className="text-neutral-500 text-xs mt-1">Creator-scoped volume index is not wired in this build</p>
            </Card>
            <Card variant="elevated" padding="lg">
              <p className="text-neutral-400 text-sm mb-1">Total Holders</p>
              <p className="text-3xl font-bold text-slate-900">Unavailable</p>
              <p className="text-neutral-500 text-xs mt-1">Holder indexing is pending</p>
            </Card>
            <Card variant="elevated" padding="lg">
              <p className="text-neutral-400 text-sm mb-1">Royalties Earned</p>
              <p className="text-3xl font-bold text-slate-900">Unavailable</p>
              <p className="text-neutral-500 text-xs mt-1">Per-creator royalty rollup is not indexed yet</p>
            </Card>
          </div>
        )}
        ...
        {activeTab === 'lock' && (
          <Card variant="elevated" padding="lg">
            <h2 className="text-2xl font-bold text-slate-900 mb-4">LP Lock Operations</h2>
            <p className="text-neutral-300 mb-6 max-w-2xl">
              LP locking is handled in the dedicated liquidity console where wallet state, lock IDs, and reward
              actions are fully wired to on-chain transactions.
            </p>
            <div className="max-w-md">
              <Link href="/liquidity" className="block">
                <Button variant="primary" size="lg" fullWidth>
                  Open Liquidity Manager
                </Button>
              </Link>
            </div>
          </Card>
