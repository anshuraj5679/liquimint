...
contract BondingCurveToken is ERC20, ReentrancyGuard {
    ...
    function buy() external payable nonReentrant {
        ...
        uint256 amountAfterFee = _takeFees(msg.value);
        ...
        // Update factory stats; this must succeed for accounting integrity
    ...
    function graduate() external nonReentrant returns (uint256 tokenAmount, uint256 maticAmount) {
        ...
        // Quote tokens using the same curve pricing logic as a buy path.
        tokenAmount = quoteGraduationTokenAmount(maticAmount);
        require(tokenAmount > 0, "Invalid graduation quote");
    ...
    /**
     * @notice Quote tokens to mint for DEX graduation liquidity.
     * @dev Uses curve-consistent quote and falls back to spot price if needed.
     */
    function quoteGraduationTokenAmount(uint256 maticAmount) public view returns (uint256) {
        uint256 curveQuoted = calculateBuyReturn(maticAmount);
        if (curveQuoted > 0) {
            return curveQuoted;
        }
        ...
        return (maticAmount * 1e18) / spotPrice;
    }
