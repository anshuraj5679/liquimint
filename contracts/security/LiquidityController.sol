...
contract LiquidityController is Ownable, ReentrancyGuard {
    ...
    function emergencyUnlock(uint256 lockId) external nonReentrant {
        ...
        require(lock.owner != address(0), "Invalid lock ID");
        ...
        IERC20(lock.lpToken).safeTransfer(msg.sender, returnAmount);

        emit LiquidityUnlocked(lock.token, lock.lpToken, msg.sender, lockId, returnAmount);
    }
     ...
     * @param percentage Percentage of LP intended to be locked (1-100)
     */
    function lockLiquidity(
        ...
        address lockOwner = _resolveLockOwner(token, msg.sender);
        ...
        userLockCount[lockOwner][token]++;

        emit LiquidityLocked(token, lpToken, lockOwner, lockId, amount, unlockTime, percentage);
    ...
    function unlockLiquidity(uint256 lockId) external nonReentrant {
        ...
        emit LiquidityUnlocked(lock.token, lock.lpToken, msg.sender, lockId, lock.amount);
    }
    ...
    /**
     * @notice Get lock IDs owned by a user
     */
    ...
    function getTotalLockedPercentage(address token) external view returns (uint256) {
        ...
        for (uint256 i = 0; i < lockIds.length; i++) {
            ...
            if (!lock.isUnlocked && block.timestamp < lock.unlockTime) {
                totalPercentage += lock.percentage;
            }
        }
    ...
    function _resolveLockOwner(address token, address defaultOwner) private view returns (address) {
        // EOAs lock for themselves. Contract callers may lock on behalf of token creator.
        if (defaultOwner.code.length == 0) {
            return defaultOwner;
        }

        try ICreatorToken(token).creator() returns (address creatorAddress) {
